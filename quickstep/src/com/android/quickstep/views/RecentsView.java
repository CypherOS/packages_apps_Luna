...
public abstract class RecentsView<T extends BaseActivity> extends PagedView implements Insettable {
    ...
    public RecentsView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mTaskViewPool = new ViewPool(context, this, R.layout.task, 20, 10);
    ...
    public void onViewRemoved(View child) {
        ...
        if (child != mClearAllButton) {
			...
			mTaskViewPool.recycle(((TaskView) child));
        }
    }
    ...
    public void showTask(int runningTaskId) {
        if (getChildCount() == 0) {
			TaskView taskView = (TaskView) mTaskViewPool.getView();
            addView(taskView);
            addView(mClearAllButton);
    ...
    public AnimatorSet createAdjacentPageAnimForTaskLaunch(
        ...
        if (launchingCenterTask) {
            ...
            if (taskIndex - 1 >= 0) {
				...
				anim.play(ObjectAnimator.ofFloat(adjacentTask, TaskView.FULLSCREEN_PROGRESS, new float[]{1.0f}));
            }
