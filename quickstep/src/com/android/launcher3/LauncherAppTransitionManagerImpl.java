...
public class LauncherAppTransitionManagerImpl extends LauncherAppTransitionManager
    ...
    public void playIconAnimators(AnimatorSet appOpenAnimator, View v, Rect windowTargetBounds, boolean isReversed) {
		...
		if (((float) lp.topMargin) <= centerY) {
			if (Math.abs(dY) >= ((float) mLauncher.getDeviceProfile().cellHeightPx)) {
                boolean useUpwardAnimation = false;
		        ...
		        if (!(v instanceof BubbleTextView)) {
                    ...
                    useUpwardAnimation = !isReversed && ((BubbleTextView) v).shouldTextBeVisible();
                    bubbleTextView.setTextVisibility(useUpwardAnimation);
