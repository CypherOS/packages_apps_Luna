...
public class LauncherAppTransitionManagerImpl extends LauncherAppTransitionManager
     ...
     */
    private void playIconAnimators(AnimatorSet appOpenAnimator, View v, Rect windowTargetBounds, boolean isReversed) {
		...
		if ((useUpwardAnimation ^ isReversed) == 0) {
		...
		long currentDelay = new float[2];
		long newDelay;
		long alphaDuration;
		Object obj = 1.0f;
		currentDelay[0] = isReversed ? null : 1.0f;
		if (isReversed) {
			obj = null;
		}
		currentDelay[1] = obj;
        ObjectAnimator alpha = ObjectAnimator.ofFloat(mFloatingView, View.ALPHA, currentDelay);
		...
		if (isReversed) {
			alpha.setStartDelay(currentDelay);
            alpha.setDuration(alphaDuration);
        } else {
            alpha.setDuration(alphaDuration);
            alpha.setStartDelay(160 - (alphaDuration + currentDelay));
        }
        alpha.setInterpolator(LINEAR);
    ...
    private ValueAnimator getOpeningWindowAnimators(View v, RemoteAnimationTargetCompat[] targets,
        ...
        appAnimator.addUpdateListener(new MultiValueUpdateListener() {
            ...
            public void onUpdate(float percent) {
				float easePercent = (isReversed ? AGGRESSIVE_EASE_REVERSED : AGGRESSIVE_EASE).getInterpolation(percent);
