...
public class LunaAppTransitionManagerImpl extends LauncherAppTransitionManagerImpl {
    ...
    private Task loadTask(int taskId) {
        ...
        plan.preloadPlan(preloadOpts, recentsTl, -1, Utilities.getUserId());
	...
	public View findInViews(ItemOperator op, ViewGroup... views) {
        ...
        int i = 0;
        while (i < length) {
            ViewGroup container = views[i];
            int itemCount = container.getChildCount();
            int index = 0;
            while (index < itemCount) {
                views = container.getChildAt(index);
                if (op.evaluate((ItemInfo) views.getTag(), views)) {
                    return views;
                }
                index++;
            }
