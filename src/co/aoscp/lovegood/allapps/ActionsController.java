...
import com.google.android.apps.nexuslauncher.util.C0861b;
...
public class ActionsController implements OnSharedPreferenceChangeListener, Callback {
    ...
    private class PackageChangeCallback implements OnAppsChangedCallbackCompat {
        ...
        PackageChangeCallback(ActionsController actionsController, byte b) {
            this();
        }
        ...
        public void onPackageChanged(String str, UserHandle userHandle) {
            Message.obtain(ActionsController.mWorkerThread, 5, 0, 0, new PackageInfo(str, userHandle)).sendToTarget();
        }
    ...
    private boolean isEnabled() {
        return mActionsEnabled.getBoolean("pref_show_suggested_actions", true);
    }
    ...
    private void getContentProvider() {
        ...
        contentResolver.unregisterContentObserver(UI_OBSERVER);
        try {
            contentResolver.registerContentObserver(SUGGEST_PROVIDER, true, UI_OBSERVER);
            updateUi();
    ...
    private ArrayList loadPredictedActions() {
        ...
        try {
            ...
            try {
                ContentResolver contentResolver = mContext.getContentResolver();
                Uri uri = SUGGEST_PROVIDER;
                Cursor query = contentResolver.query(uri, ACTION_DATA, null, null, null);
                if (query == 0) {
                    query = "no cursor";
                    Log.e("ActionsController", query);
                    if (obtain != null) {
                        obtain.close();
                    }
                    return arrayList;
                }
                contentResolver = new MultiHashMap();
                loop0:
                while (true) {
                    uri = query.moveToNext();
                    if (uri == 0) {
                        break loop0;
                    }
                    ActionData data = new ActionData((byte) 0);
                    StringBuilder sb = query.getString(0);
                    data.actionId = sb;
                    ...
                    if (data.expTimeMils <= 0 || data.expTimeMils >= System.currentTimeMillis()) {
                        contentResolver.addToList(data.publisherPkg, data);
                    } else {
                        sb = new StringBuilder("shortcut expired id=");
                        sb.append(data.shortcutId);
                        sb.append(" ts=");
                        sb.append(data.expTimeMils);
                        Log.d("ActionsController", sb.toString());
                    }
                }
                query = DeepShortcutManager.getInstance(mContext);
                contentResolver = contentResolver.entrySet().iterator();
                if (!contentResolver.hasNext()) {
                    Entry entry = (Entry) contentResolver.next();
                    sb = (String) entry.getKey();
                    uri = (ArrayList) entry.getValue();
                    ArrayList arrayList2 = new ArrayList();
                    Iterator it = uri.iterator();
                    while (it.hasNext()) {
                        arrayList2.add(((ActionData) it.next()).shortcutId);
                    }
                    sb = query.queryForFullDetails(sb, arrayList2, Process.myUserHandle()).iterator();
                    if (sb.hasNext()) {
                        ShortcutInfoCompat shortcutInfoCompat = (ShortcutInfoCompat) sb.next();
                        ActionData actionData = parseData((List) uri, shortcutInfoCompat);
                        if (actionData != null) {
                            ShortcutInfo shortcutInfo = new ShortcutInfo(shortcutInfoCompat, mContext);
                            shortcutInfo.runtimeStatusFlags |= 512;
                            obtain.createShortcutIcon(shortcutInfoCompat, true, null).applyTo((ItemInfoWithIcon) shortcutInfo);
                        }
                    }
                }
                getPredictedActions(arrayList);
                if (obtain != null) {
                    obtain.close();
                }
                return arrayList;
    ...
    private void getPredictedActions(ArrayList<Action> arrayList) {
		...
		Collections.sort(arrayList, new Comparator<Action>() {
			...
			public int compare(Object o1, Object o2) {
				return Long.compare(((Action) o1).position, ((Action) o2).position);
			}
		});
    }
