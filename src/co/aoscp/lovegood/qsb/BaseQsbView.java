...
public abstract class BaseQsbView extends FrameLayout implements OnSharedPreferenceChangeListener, OnClickListener, OnLongClickListener, Insettable {
	...
	private Bitmap mNewShadowBitmap;
    ...
    public final void updateMicView() {
        ...
        mMicIconView.setImageDrawable(mContext.getResources().getDrawable(R.drawable.ic_poodle_color););
    ...
    public void onLayout(boolean z, int i, int i2, int i3, int i4) {
        ...
        if (mIsRtl) {
            rect = mSrcRect;
            rect.left -= mShadowMargin;
        } else {
            rect = mSrcRect;
            rect.right += mShadowMargin;
        }
        TransformingTouchDelegate transformingTouchDelegate = mQsbDelegate;
        Rect rect2 = mSrcRect;
        transformingTouchDelegate.setBounds(rect2.left, rect2.top, rect2.right, rect2.bottom);
    }
    ...
    public void setColorAlpha(int newColor) {
        mNewColor = newColor;
        if (mNewColor != mColor || mNewShadowBitmap != mShadowBitmap) {
            mNewShadowBitmap = null;
            invalidate();
        }
    }
        ...
        if (mUseTwoBubbles) {
            ...
            if (mNewShadowBitmap == null) {
                ...
                if (shadowDimens != 0) {
                    ...
                    bitmap = getShadowBitmap(mNewColor);
                }
                mNewShadowBitmap = bitmap;
            }
            Bitmap newBitmap = mNewShadowBitmap;
