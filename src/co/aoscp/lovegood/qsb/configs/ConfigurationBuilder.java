...
public class ConfigurationBuilder {
    ...
    public ConfigurationBuilder(BaseQsbView qsbView, boolean isAllApps) {
		...
		mLongClickReceiver = new BroadcastReceiver() {
			public WeakReference<LunaLauncher> mWeakReference = new WeakReference(null);

			public void getWeakReference(LunaLauncher launcher) {
    ...
    public byte[] build() {
        ...
        mBundle.putParcelable(mNano.er, PendingIntent.getBroadcast(
		        mLauncher, 2055, new Intent().setComponent(new ComponentName(mLauncher, "LongClickReceiver")), 
		...
		public PendingItemDragHelper createDragHelper() {
			PendingAddShortcutInfo tag = new PendingAddShortcutInfo(new ShortcutConfigActivityInfoVO(mActivityInfo) {
				public ShortcutInfo createShortcutInfo() {
					return InstallShortcutReceiver.fromActivityInfo(ItemDragListener.mActivityInfo, ItemDragListener.mLauncher);
				}
			});
