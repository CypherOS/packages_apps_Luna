...
public class AllAppsQsbView extends BaseQsbView implements SearchUiManager, OnChangeListener {
    ...
    protected void setSearchType() {
		int orient;
        boolean useFallbackSearch = false;
        if (mContext.getResources().getConfiguration().orientation == LinearLayout.HORIZONTAL) {
			orient = 16;
        } else {
            orient = 8;
        }
		...
		if (mUseFallbackSearch != useFallbackSearch) {
            removeFallbackView();
			mUseFallbackSearch = useFallbackSearch;
