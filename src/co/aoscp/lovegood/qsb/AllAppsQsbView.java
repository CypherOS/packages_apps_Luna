...
public class AllAppsQsbView extends BaseQsbView implements SearchUiManager, OnChangeListener {
    ...
    public boolean mUseFallbackSearch = false;

    public AllAppsQsbView(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    public AllAppsQsbView(Context context, AttributeSet attributeSet, int i) {
        super(context, attributeSet, i);
        mShadowAlpha = 0;
        setOnClickListener(this);
        mQsbConfig = QsbConfiguration.getInstance(context);
        mMarginAdjusting = getResources().getDimensionPixelSize(R.dimen.qsb_margin_top_adjusting);
    ...
    public void initialize(AllAppsContainerView allAppsContainerView) {
        ...
        mAppsView.addElevationController(new BroadcastReceiver() {
			@Override
    ...
    public void updateConfiguration() {
        ...
        mQsbConfig.micStrokeWidth();
        setMicPaint(0.0f);
        mQsbConfig.hintIsForAssistant();
        mQsbConfig.useTwoBubbles();
    ...
    public void startSearch(String initialQuery, int i) {
		if (mUseFallbackSearch) {
            mDefaultSearchView.setText(initialQuery);
            return;
        }
        ...
        if (mLauncher.getLauncherCallbacks().startSearch(config.build(), false, config.getExtras())) {
            ...
            mDefaultSearchView.mShowImeAfterFirstLayout = mDefaultSearchView.showSoftInput() ^ 1;
        }
		mResult = 0
    }
    ...
    public void ensureFallbackView() {
        ...
        mDefaultSearchView.f427DI.initialize(searchThread, mDefaultSearchView, Launcher.getLauncher(mDefaultSearchView.getContext()), mDefaultSearchView);
        addView(mDefaultSearchView);
    }

    public void removeFallbackView() {
        if (mDefaultSearchView != null) {
    ...
    public void draw(Canvas canvas) {
        if (mShadowAlpha > 0) {
            if (mQsbScroll == null) {
                mQsbScroll = createBitmap(getResources().getDimension(
				        R.dimen.hotseat_qsb_scroll_shadow_blur_radius), getResources().getDimension(R.dimen.hotseat_qsb_scroll_key_shadow_offset), 0);
            }
    ...
    public void setSearchType() {
        if (getResources().getConfiguration().orientation == HORIZONTAL) {
            mUseFallbackSearch = true;
        }
        if (!mUseFallbackSearch) {
            removeFallbackView();
            ((ImageView) findViewById(R.id.g_icon)).setImageResource(mUseFallbackSearch ? R.drawable.ic_allapps_search : R.drawable.ic_super_g_color);
            mQsbDelegate.setAlpha(mUseFallbackSearch ? 0.0f : 1.0f);
            if (mUseFallbackSearch) {
                ensureFallbackView();
