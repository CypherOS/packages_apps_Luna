...
public class QuickspaceEventController {
	...
	private void checkForFirstTime() {
		...
		if (mIsFirstTimeDone) {
			if (mListener != null) {
				mListener.onNewEvent(deviceIntro, null, false);
			}
		} else {
			if (mListener != null) {
				mListener.onNewEvent(null, null, false);
			}
		}
	}
    ...
    private void updateEventInfo() {
		...
		if (mListener != null && !mIsFirstTimeDone) {
            mListener.onNewEvent(data.getSongTitle(), data.getArtistTitle(), true);
	...
	private class FirstTimeSettingsObserver extends ContentObserver {
        ...
        public void update() {
            mIsFirstTimeDone = Settings.System.getIntForUser(mContext.getContentResolver(),
