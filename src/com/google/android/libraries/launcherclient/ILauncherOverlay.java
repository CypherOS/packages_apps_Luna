...
public interface ILauncherOverlay extends IInterface {
    ...
    public abstract class Stub extends Binder implements ILauncherOverlay {
        ...
        public static class Proxy implements ILauncherOverlay {
			...
			public void windowAttached(Bundle bundle, ILauncherOverlayCallback cb) throws RemoteException {
                Parcel _data = Parcel.obtain();
                try {
                    _data.writeInterfaceToken(Stub.DESCRIPTOR);
                    if (bundle != null) {
            ...
            if (code != 1598968902) {
                switch (code) {
                    ...
                    case WINDOW_ATTACHED_TRANSACTION:
                        data.enforceInterface(descriptor);
                        Bundle waBundle = new Bundle();
                        if (data.readInt() != 0) {
                            waBundle = (Bundle) Bundle.CREATOR.createFromParcel(data);
                        }
                        windowAttached(waBundle, com.google.android.libraries.launcherclient.ILauncherOverlayCallback.Stub.asInterface(data.readStrongBinder()));
                        return true;
                    ...
                    case START_SEARCH_TRANSACTION:
                        ...
                        if (data.readInt() != 0) {
                            searchBundle = (Bundle) Bundle.CREATOR.createFromParcel(data);
                        }
                        int result = startSearch(dataArray, searchBundle) ? 1 : 0;
