...
public class CustomIconProvider extends DynamicIconProvider implements Runnable {
    ...
    @Override
    public void run() {
        mIconPack = Utilities.getPrefs(mContext).getString(Utilities.ICON_PACK_PREF, "");
        if (mIconsHandler.isPackProvider(mContext, mIconPack)) {
            try {
    ...
    public Drawable getIcon(LauncherActivityInfo launcherActivityInfo, int iconDpi, boolean flattenDrawable) {
		...
		Bitmap bitMap = mIconsHandler.getDrawableIconForPackage(launcherActivityInfo.getComponentName());
        if (bitMap == null) {
            return launcherActivityInfo.getIcon(iconDpi);
        }
        ...
        return new BitmapDrawable(mContext.getResources(), LauncherIcons.createIconBitmap(bitMap, mContext));
    }
