...
public class DynamicIconProvider extends IconProvider {
    private final BroadcastReceiver mBroadcastReceiver = new DynamicIconProviderReceiver(this);
    ...
    public Drawable getIcon(LauncherActivityInfo launcherActivityInfo, int iconDpi, boolean icon) {
        ...
        if (isCalendar(str)) {
            try {
                Bundle bundle = mPackageManager.getActivityInfo(launcherActivityInfo.getComponentName(), PackageManager.GET_META_DATA | PackageManager.MATCH_UNINSTALLED_PACKAGES).metaData;
        ...
        } else if (!icon && ClockStatusReceiver.fk.equals(launcherActivityInfo.getComponentName()) && Process.myUserHandle().equals(launcherActivityInfo.getUser())) {
