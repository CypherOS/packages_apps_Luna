...
public class UserEventDispatcher implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static UserEventDispatcher newInstance(Context context, DeviceProfile dp,
		...
		ued.mContext = context;
		ued.mPrefs = Utilities.getDevicePrefs(context);
        ued.mPrefs.registerOnSharedPreferenceChangeListener(ued);
        ued.mPackageManager = context.getPackageManager();
        String uuidStr = ued.mPrefs.getString(UUID_STORAGE, null);
        if (uuidStr == null) {
            uuidStr = UUID.randomUUID().toString();
            ued.mPrefs.edit().putString(UUID_STORAGE, uuidStr).apply();
        }
