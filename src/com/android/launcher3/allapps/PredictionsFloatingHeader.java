...
import com.android.launcher3.PredictionUiStateManager.AppPredictionConsumer;
...
public class PredictionsFloatingHeader extends FloatingHeaderView implements Insettable, AppPredictionConsumer {
    ...
    public PredictionsFloatingHeader(Context context, AttributeSet attributeSet) {
        ...
        mHeaderTopPadding = context.getResources().getDimensionPixelSize(2131165276);
        mPredictionUiStateManager = PredictionUiStateManager.getInstance(context);
    }

    public void onFinishInflate() {
        super.onFinishInflate();
        mPredictionRowView = (PredictionRowView) findViewById(2131427603);
    ...
    public final void updateExpectedHeight() {
        ...
        if (mShowAllAppsLabel && mTabsHidden) {
    ...
    public int getMaxTranslation() {
        if (mMaxTranslation == 0 && mTabsHidden) {
            return getResources().getDimensionPixelSize(2131165285);
        }
    ...
    public void setPredictedApps(boolean enabled, List<ComponentKeyMapper> list) {
        mPredictionRowView.setPredictedApps(enabled, list);
    }
}
