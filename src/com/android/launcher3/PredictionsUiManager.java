...
public class PredictionsUiManager implements OnSharedPreferenceChangeListener {
    ...
    private void clearNonExistingComponents() {
        ...
        for (String prediction : originalSet) {
            try {
				components = new ComponentKey(mContext, prediction).componentName;
                mPackageManager.getActivityInfo(components, 0);
                ...
                edit.remove(sb.toString());
				if (components != null) {
					Intent intent = mPackageManager.getLaunchIntentForPackage(components.getPackageName());
					if (intent != null) {
						ComponentName componentInfo = intent.getComponent();
						if (componentInfo != null) {
