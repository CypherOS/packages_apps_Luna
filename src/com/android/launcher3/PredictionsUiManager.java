...
public class PredictionsUiManager implements OnSharedPreferenceChangeListener {
	...
	private static final String TAG = "PredictionsUiManager";
    ...
    public List<ComponentKeyMapper> getPredictedApps() {
		final String[] otherWatchedApps = mContext.getResources().getStringArray(R.array.config_otherWatchedApps);
        ...
        if (isPredictorEnabled()) {
            ...
            for (int i = 0; i < WATCHED_APPS.length && list.size() < MAX_PREDICTIONS; i++) {
				String watchedApps = WATCHED_APPS[i] + otherWatchedApps[i];
    ...
    private boolean isPredictorEnabled() {
        return Utilities.getPrefs(mContext).getBoolean(SettingsActivity.KEY_APP_SUGGESTIONS, true);
    }
