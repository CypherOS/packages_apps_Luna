...
public class PredictionsUiManager implements OnSharedPreferenceChangeListener {
	...
	protected final UserHandle mUser = Process.myUserHandle();
    ...
    public List<ComponentKeyMapper<AppInfo>> getPredictedApps() {
        ...
        if (isPredictorEnabled()) {
            ...
            Collections.sort(predictionList, new PredictionsObserver(this));
    ...
    public void logAppLaunch(View v, Intent intent) {
        if (isPredictorEnabled() && recursiveIsDrawer(v)) {
            ComponentName componentInfo = intent.getComponent();
            if (componentInfo != null && mAppFilter.shouldShowApp(componentInfo, mUser)) {
                clearNonExistingComponents();
                Set<String> predictionSet = getStringSetCopy();
                Editor edit = mPrefs.edit();
                String prediction = new ComponentKey(componentInfo, mUser).toString();
    ...
    private void clearNonExistingComponents() {
        ...
        for (String prediction : originalSet) {
            ComponentName cn = new ComponentKey(prediction, mUser).componentName;
            try {
	...
	public class PredictionsObserver implements Comparator {

		private PredictionsUiManager mUiManager;

		public PredictionsObserver(PredictionsUiManager uiManager) {
			mUiManager = uiManager;
		}

		public final int compare(Object obj, Object obj2) {
			return Integer.compare(mUiManager.getLaunchCount((String) obj2), mUiManager.getLaunchCount((String) obj));
		}
	}
}
