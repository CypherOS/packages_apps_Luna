...
public class QuickEvents {
	...
	private boolean mWasUsingAmbientPlay = false;
    ...
    public QuickEvents(Context context) {
		...
		mAmbientObserver = new AmbientObserver(
                mHandler, context.getContentResolver());
        mAmbientObserver.register();
        mAmbientObserver.updateState();
        initQuickEvents();
    }

	public void initAmbientObserver() {
		if (!mAmbientEnabled) {
			if (mWasUsingAmbientPlay) {
				mAmbientManager.unRegisterCallback(mAmbientCallback);
				mWasUsingAmbientPlay = false;
			}
			Log.d("QuickEvents", "Removing AmbientIndication Callbacks");
			return;
		}
		mAmbientManager = new AmbientManager(mContext);
        mAmbientManager.registerCallback(mAmbientCallback);
		mWasUsingAmbientPlay = true;
	}
	...
	private AmbientIndicationManagerCallback mAmbientCallback = new AmbientIndicationManagerCallback() {
        ...
        public void onRecognitionResult(DataObserver observed) {
            if (observed.Song != null && observed.Artist != null) {
				mEventTitleText = Integer.parseInt(String.format(mContext.getResources().getString(
				...
				Log.d("QuickEvents", "Got Ambient info");
			}
        }
	...
	private class AmbientObserver extends ContentObserver {

        private Handler mHandler;
        private ContentResolver mResolver;

        AmbientObserver(Handler handler, ContentResolver resolver) {
            super(handler);
            mHandler = handler;
            mResolver = resolver;
        }
        ...
        public void register() {
            mResolver.registerContentObserver(Settings.System.getUriFor(
                    SETTING_AMBIENT_RECOGNITION), false, this);
        }
        ...
        public void updateState() {
            mAmbientEnabled = Settings.System.getInt(mResolver, SETTING_AMBIENT_RECOGNITION, 0) != 0;
        }
    }
}