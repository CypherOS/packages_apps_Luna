...
public class ViewPool {
    ...
    private void initPool(int count) {
        new Thread(new ViewPoolOffload(count, new Handler())).start();
    }
    ...
    public static void startRecycle(int count, Handler handler) {
        for (int i = 0; i < count; i++) {
			handler.post(new Runnable() {
				@Override
				public void run() {
                    sViewPool.postRecycle();
				}
			});
        }
    }
