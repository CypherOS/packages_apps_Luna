 ...
 */
public class LunaDrawableFactory {

    private static final String TAG = "LunaDrawableFactory";

	private static LunaDrawableFactory sInstance;
    private static final Object LOCK = new Object();

    private Path mPreloadProgressPath;
	
	private DynamicClock mDynamicClock;
	...
	public LunaDrawableFactory(Context context) {
	    ...
	    mDynamicClock = new DynamicClock(context);
    }

    public static LunaDrawableFactory get(Context context) {
		synchronized (LOCK) {
            if (sInstance == null) {
                sInstance = Utilities.getOverrideObject(LunaDrawableFactory.class,
    ...
    public FastBitmapDrawable newIcon(Bitmap icon, ItemInfo info) {
		/**if (info != null &&
                ...
                info.user.equals(Process.myUserHandle()) && !hasIconPack) {
            return mDynamicClock.drawIcon(icon);
		}**/ // Disable until NPE is resolved
		if (hasIconPack) {
