package com.android.launcher3.backports.allapps;

/* renamed from: com.google.android.apps.nexuslauncher.allapps.i */
class C0834i {
    /* renamed from: Bl */
    int f1226Bl;
    /* renamed from: Bm */
    String f1227Bm;
    /* renamed from: Bn */
    int f1228Bn;
    /* renamed from: Bo */
    String f1229Bo;
    /* renamed from: ts */
    long f1230ts;

    private C0834i() {
    }

    /* synthetic */ C0834i(byte b) {
        this();
    }

    public String toString() {
        StringBuilder stringBuilder = new StringBuilder("eventType:");
        stringBuilder.append(this.f1226Bl);
        stringBuilder.append(" clickedId:");
        stringBuilder.append(this.f1227Bm);
        stringBuilder.append(" clickedPos:");
        stringBuilder.append(this.f1228Bn);
        stringBuilder.append(" top:");
        stringBuilder.append(this.f1229Bo);
        return stringBuilder.toString();
    }
}
