...
public class DeepShortcutManager {
    ...
    public List queryForFullDetails(String str, List list, UserHandleCompat userHandle) {
        return query(11, str, null, list, userHandle);
    }

    public List queryForShortcutsContainer(ComponentName componentName, List list, UserHandleCompat userHandle) {
    ...
    public void unpinShortcut(ShortcutKey shortcutKey) {
        if (Utilities.isNycOrAbove()) {
            ...
            UserHandleCompat userHandle = shortcutKey.user;
			UserHandle userHandle2 = Utilities.myUserHandle();
            List extractIds = extractIds(queryForPinnedShortcuts(packageName, userHandle));
            extractIds.remove(id);
            try {
                mLauncherApps.pinShortcuts(packageName, extractIds, userHandle2);
    ...
    public void startShortcut(String str, String str2, Rect rect, Bundle bundle, UserHandleCompat userHandle) {
		UserHandle userHandle2 = Utilities.myUserHandle();
        if (Utilities.isNycMR1OrAbove()) {
            try {
                mLauncherApps.startShortcut(str, str2, rect, bundle, userHandle2);
                mWasLastCallSuccess = true;
            } catch (Throwable e) {
    ...
    public List queryForPinnedShortcuts(String str, UserHandleCompat userHandle) {
        return query(2, str, null, null, userHandle);
    }

    public List queryForAllShortcuts(UserHandleCompat userHandle) {
    ...
    private List query(int i, String str, ComponentName componentName, List list, UserHandleCompat userHandle) {
		UserHandle userHandle2 = Utilities.myUserHandle();
        ...
        try {
            iterable = this.mLauncherApps.getShortcuts(shortcutQuery, userHandle2);
            mWasLastCallSuccess = true;
        } catch (Throwable e) {
