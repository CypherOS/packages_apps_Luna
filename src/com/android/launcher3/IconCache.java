...
public class IconCache {
    ...
    @Thunk void addIconToDBAndMemCache(LauncherActivityInfo app, PackageInfo info,
            long userSerial) {
    ...
    @Thunk synchronized void addIconToDBAndMemCache(LauncherActivityInfo app,
        ...
        if (entry == null) {
            entry = new CacheEntry();
            entry.icon = LauncherIcons.createBadgedIconBitmap(mIconProvider.getIcon(app, mIconDpi, true),
	...
	@Thunk ContentValues updateCacheAndGetContentValues(LauncherActivityInfo app,
        ...
        if (entry == null) {
            entry = new CacheEntry();
            entry.icon = LauncherIcons.createBadgedIconBitmap(mIconProvider.getIcon(app, mIconDpi, true),
                    app.getUser(), mContext, Build.VERSION_CODES.O);
        }
        entry.title = app.getLabel();
        entry.contentDescription = mUserManager.getBadgedLabelForUser(entry.title, app.getUser());
